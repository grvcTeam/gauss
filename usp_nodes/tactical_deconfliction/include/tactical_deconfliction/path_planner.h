...
#ifndef KML_PARSER_H
/// Dummy KMLparser class used when USING_IN_MULTIDRONE_PROJECT is not defined.
class KMLparser {
public:
    KMLparser() {}
    ~KMLparser() {}
    void runParser(const std::string& _KML_string) {}
...
public:
    ...
    PathPlanner(const std::string& _KML_string, unsigned int _max_grid_side = 100);  // Constructor that generates the grid of obstacles (no_fly_zones_) from the KML information. The input is the string that contains the KML and the size (in number of cells) of the bigger side of the rectanglular grid of obstacles, predefined at 100. IMPORTANT: the limits of the KML are expanded with an empty (without obstacles) row or column at each side of the KML no_fly_zone_ map, if this is a problem just add obstacles at the borders of the matrix map.
    PathPlanner(const std::vector<geometry_msgs::Polygon>& _obstacle_polygon_vector, unsigned int _max_grid_side = 100);   // Constructor that does the same that the constructor with KML but receiving the obstacles list directly in a polygon vector, not inside a KML.
