...
PathPlanner::PathPlanner(const std::vector< std::vector<bool> >& _no_fly_zones, double _min_x, double _max_x, double _min_y, double _max_y) { // _no_fly_zones is the received grid obstacle matrix. _min_x, _max_x, _min_y, _max_y are the x-y and coordinates of the limits of the grid.
    ...
    KML_parser_from_path_planner_.min_x_ = _min_x - x_cell_width_;
    KML_parser_from_path_planner_.max_x_ = _max_x + x_cell_width_;
    KML_parser_from_path_planner_.min_y_ = _min_y - y_cell_width_;
    KML_parser_from_path_planner_.max_y_ = _max_y + y_cell_width_;
...
PathPlanner::PathPlanner(const std::string& _KML_string, unsigned int _max_grid_side) { // _max_grid_side is the number of cells that the grid has in the bigger side of the rectangular map of obstacles (predefined in the header file). KML_string is the string that contains the KML.
    ...
    KML_parser_from_path_planner_.runParser( _KML_string );
...
PathPlanner::PathPlanner(const std::vector<geometry_msgs::Polygon>& _obstacle_polygon_vector, unsigned int _max_grid_side) { // _max_grid_side is the number of cells that the grid has in the bigger side of the rectangular map of obstacles (predefined in the header file). KML_string is the string that contains the KML.
...
std::vector<geometry_msgs::PointStamped> PathPlanner::getPath(const geometry_msgs::PointStamped& _initial_point_stamped, const geometry_msgs::PointStamped& _final_point_stamped, bool _movement_pattern, bool _show_results) {
    ...
    if ( (_initial_point_stamped.point.x<KML_parser_from_path_planner_.min_x_-x_cell_width_) || (_initial_point_stamped.point.y<KML_parser_from_path_planner_.min_y_-y_cell_width_) || (_initial_point_stamped.point.x>KML_parser_from_path_planner_.max_x_+x_cell_width_) || (_initial_point_stamped.point.y>KML_parser_from_path_planner_.max_y_+y_cell_width_) || (_final_point_stamped.point.x<KML_parser_from_path_planner_.min_x_-x_cell_width_) || (_final_point_stamped.point.y<KML_parser_from_path_planner_.min_y_-y_cell_width_) || (_final_point_stamped.point.x>KML_parser_from_path_planner_.max_x_+x_cell_width_) || (_final_point_stamped.point.y>KML_parser_from_path_planner_.max_y_+y_cell_width_) ) {
        if ( ( (_initial_point_stamped.point.x<KML_parser_from_path_planner_.min_x_-x_cell_width_) || (_initial_point_stamped.point.y<KML_parser_from_path_planner_.min_y_-y_cell_width_) || (_initial_point_stamped.point.x>KML_parser_from_path_planner_.max_x_+x_cell_width_) || (_initial_point_stamped.point.y>KML_parser_from_path_planner_.max_y_+y_cell_width_) ) && ( (_final_point_stamped.point.x<KML_parser_from_path_planner_.min_x_-x_cell_width_) || (_final_point_stamped.point.y<KML_parser_from_path_planner_.min_y_-y_cell_width_) || (_final_point_stamped.point.x>KML_parser_from_path_planner_.max_x_+x_cell_width_) || (_final_point_stamped.point.y>KML_parser_from_path_planner_.max_y_+y_cell_width_) ) )
            std::cout << std::endl << "Error, initial and final points out of the KML map." << std::endl << std::endl;
        else if ( (_initial_point_stamped.point.x<KML_parser_from_path_planner_.min_x_-x_cell_width_) || (_initial_point_stamped.point.y<KML_parser_from_path_planner_.min_y_-y_cell_width_) || (_initial_point_stamped.point.x>KML_parser_from_path_planner_.max_x_+x_cell_width_) || (_initial_point_stamped.point.y>KML_parser_from_path_planner_.max_y_+y_cell_width_) )
            std::cout << std::endl << "Error, initial point out of the KML map." << std::endl << std::endl;
        else if ( (_final_point_stamped.point.x<KML_parser_from_path_planner_.min_x_-x_cell_width_) || (_final_point_stamped.point.y<KML_parser_from_path_planner_.min_y_-y_cell_width_) || (_final_point_stamped.point.x>KML_parser_from_path_planner_.max_x_+x_cell_width_) || (_final_point_stamped.point.y>KML_parser_from_path_planner_.max_y_+y_cell_width_) )
            std::cout << std::endl << "Error, final point out of the KML map." << std::endl << std::endl;
        return path;
    }
    ...
    if ( (no_fly_zones_[initial_point_in_grid_y][initial_point_in_grid_x]==1)||(no_fly_zones_[final_point_in_grid_y][final_point_in_grid_x]==1) ) {
        if ( (no_fly_zones_[initial_point_in_grid_y][initial_point_in_grid_x]==1)&&(no_fly_zones_[final_point_in_grid_y][final_point_in_grid_x]==1) ) std::cout << std::endl << "No path found, initial and final points inside obstacles." << std::endl << std::endl;
        else if ( no_fly_zones_[initial_point_in_grid_y][initial_point_in_grid_x]==1 ) std::cout << std::endl << "No path found, initial point inside an obstacle." << std::endl << std::endl;
        else if ( no_fly_zones_[final_point_in_grid_y][final_point_in_grid_x]==1 ) std::cout << std::endl << "No path found, final point inside an obstacle." << std::endl << std::endl;
    ...
    if ( path.size()==0 ) {
        std::cout << std::endl << "Path Planner: No path possible between the initial and final points, even though those two are obstacle-free. Problem of too low grid resolution?" << std::endl << std::endl;
...
std::vector<geometry_msgs::PointStamped> PathPlanner::getPathWithTimePredictions(const geometry_msgs::PointStamped& _initial_point_stamped, const geometry_msgs::PointStamped& _final_point_stamped, int _full_speed_xy, int _full_speed_z_down, int _full_speed_z_up, bool _movement_pattern, bool _show_results) {
    ...
    if ( _final_point_stamped.header.stamp.sec < _initial_point_stamped.header.stamp.sec ) {
        ...
        if ( first_moving_time_xy >= moving_time_z ) {
            ...
            double aux_path_flat_distance_ = sqrt(pow(path_stamped[0].point.x-_initial_point_stamped.point.x,2)+pow(path_stamped[0].point.y-_initial_point_stamped.point.y,2));
            for (int i=0; i<path_stamped.size()-1; i++) {
                path_stamped[i].header.stamp.sec = _initial_point_stamped.header.stamp.sec + time_difference * aux_path_flat_distance_/distance_xy;                 // Estimate time by distance done at the waypoint in proportion with the total and the time difference.
                aux_path_flat_distance_ += sqrt(pow(path_stamped[i].point.x-path_stamped[i+1].point.x,2)+pow(path_stamped[i].point.y-path_stamped[i+1].point.y,2)); // Update distance done at the waypoint.
            }
        } else {
            ...
            double aux_path_flat_distance_ = sqrt(pow(path_stamped[1].point.x-path_stamped[0].point.x,2)+pow(path_stamped[1].point.y-path_stamped[0].point.y,2));
            for (int i=1; i<path_stamped.size()-1; i++) {
                path_stamped[i].header.stamp.sec = path_stamped[0].header.stamp.sec + time_difference * aux_path_flat_distance_/(distance_xy-first_distance_xy);    // Estimate time by distance done at the waypoint in proportion with the total and the time difference.
                aux_path_flat_distance_ += sqrt(pow(path_stamped[i].point.x-path_stamped[i+1].point.x,2)+pow(path_stamped[i].point.y-path_stamped[i+1].point.y,2)); // Update distance done at the waypoint.
            }
        }
